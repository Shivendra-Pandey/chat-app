...
function AddUser(){
    ...
    const {chats, user, selectedChat, setSelectedChat, setAddMembers, addMembers} = useContext(UserContext);
    ...
    const handleAddUser= async () =>{  
        const response = await fetch("http://localhost:5000/api/chat/adduser",{
            method : "POST",
    ...
    return(
            ...
            <button className="bg-green-500 text-white px-2 py-2 " onClick={handleAddUser}>Add Member</button>
          </div>
    )
}
